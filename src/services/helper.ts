import {ServerErrorData} from '../infra/http/type';
...
export const serverErrorDataToString = (
  ...
  const {detail, message} = serverError;
  if (!detail && !message) {
    return 'Server error, please try again later.';
  }

  if (message) {
    // because server return message : "Request failed with status code 502..."
...
export enum APIErrorCode {
  ...
  SCORE_BOARD_NOT_FOUND = 'SCORE_BOARD_NOT_FOUND',
  ...
  /** User is disabled/locked out by operator */
  ...
  /** Other types of error that never should happen. */
